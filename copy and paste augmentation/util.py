...
def issmallobject(bbox, thresh):
    if ((bbox[0]  <= thresh) & (bbox[1]<thresh)):
        return True
    else:
        return False
...
def rescale_yolo_labels(labels, img_shape):
    ...
    for box in list(labels):
        x_c = float(box[0]) * width #x1
        y_c = float(box[1]) * height #y1
        w = float(box[2]) * width #x2
        h = float(box[3]) * height #y2
        x_left = x_c - w * .5
        y_left = y_c - h * .5
        x_right = x_c + w * .5
        y_right = y_c + h * .5

        #largest size bounding box
        x1 = min(int(float(box[0])),int(float(box[2])) ,int(float(box[4])), int(float(box[6])))
        x2 = max(int(float(box[0])), int(float(box[2])), int(float(box[4])), int(float(box[6])))
        y1 = min(int(float(box[1])), int(float(box[3])), int(float(box[5])), int(float(box[7])))
        y2 = max(int(float(box[1])), int(float(box[3])), int(float(box[5])), int(float(box[7])))

        #rescale_boxes.append([box[8], int(float(box[0])), int(float(box[1])), int(float(box[4])), int(float(box[5]))])
        rescale_boxes.append([box[8], x1, y1, x2, y2])
...
def bbox_iou(box1, box2):
    ...
    if inter_width > 0 and inter_height > 0:  # strong condition
        ...
        #print(inter_area, b1_area, b2_area)
    else:
        iou = 0
    #print(iou)
    return iou
    ...
    while (success_num < paste_number):
        ...
        if max(ious) <= iou_thresh:
            ...
            for bbox_t in rescale_boxes:
                global iou
                iou =  bbox_iou(new_bbox[0:],bbox_t[0:])
        if(iou <= iou_thresh):
            success_num += 1

...
def convert_all_boxes(shape, anno_infos, yolo_label_txt_dir):
    ...
    for anno_info in anno_infos:
        ...
        #print(bb)
        label_file.write(str(target_id) + " " + " ".join([str(a) for a in b]) + '\n')
...
def copysmallobjects(image_dir, label_dir, save_base_dir, save_crop_base_dir=None,
    ...
    #     yolo_txt_dir = join(save_dir, basename(image_dir.replace('.png', '_augment.txt')))
    #     cv2.imwrite(join(save_dir, basename(image_dir).replace('.png', '_augment.png')), image)
    #     convert_all_boxes(image.shape, rescale_labels, yolo_txt_dir)
    #     return
    #print(len(labels))
    rescale_labels = rescale_yolo_labels(labels, image.shape)  # 转换坐标表示
    all_boxes = []
    ...
    for idx, rescale_label in enumerate(rescale_labels):
        ...
        if (issmallobject((rescale_label_height, rescale_label_width), thresh=32)): #small object인지 아닌지

            #print(rescale_label)
            roi = image[rescale_label[2]:rescale_label[4], rescale_label[1]:rescale_label[3]]

            new_bboxes = random_add_patches(rescale_label, rescale_labels, image.shape, paste_number=2, iou_thresh=0.5)
            count = 0

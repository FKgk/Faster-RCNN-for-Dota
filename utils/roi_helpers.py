...
def calc_iou(R, img_data, C, class_mapping):
    ...
    for ix, (x1, y1, x2, y2) in enumerate(R):
        # (x1, y1, x2, y2) = R[ix, :]
        x1 = int(round(x1))
        y1 = int(round(y1))
        x2 = int(round(x2))
        ...
        if best_iou < C.classifier_min_overlap:
            # print(best_iou, " < ", C.classifier_min_overlap)
            continue
        else:
            w = x2 - x1
            h = y2 - y1
...
def rpn_to_roi(rpn_layer, regr_layer, cfg, dim_ordering='tf', use_regr=True, max_boxes=300, overlap_thresh=0.9):
    regr_layer = regr_layer / cfg.std_scaling # std_scaling == 4.0
    ...
    curr_layer = 0
    A = np.zeros((4, rpn_layer.shape[1], rpn_layer.shape[2], rpn_layer.shape[3])) # (4, 14, 14, (9*4))
